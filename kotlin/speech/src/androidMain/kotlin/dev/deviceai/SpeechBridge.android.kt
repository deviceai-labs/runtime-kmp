...
actual object SpeechBridge {
    ...
    actual fun initStt(modelPath: String, config: SttConfig): Boolean = nativeInitStt(
        modelPath,
        config.language,
    ...
    actual fun transcribe(audioPath: String): String = nativeTranscribe(audioPath)

    actual fun transcribeDetailed(audioPath: String): TranscriptionResult = nativeTranscribeDetailed(audioPath)

    actual fun transcribeAudio(samples: FloatArray): String = nativeTranscribeAudio(samples)

    actual fun transcribeStream(samples: FloatArray, callback: SttStream) = nativeTranscribeStream(samples, callback)
    ...
    actual fun initTts(modelPath: String, configPath: String, config: TtsConfig): Boolean = nativeInitTts(
        modelPath,
        configPath,
    ...
    actual fun synthesize(text: String): ShortArray = nativeSynthesize(text)

    actual fun synthesizeToFile(text: String, outputPath: String): Boolean = nativeSynthesizeToFile(text, outputPath)

    actual fun synthesizeStream(text: String, callback: TtsStream) = nativeSynthesizeStream(text, callback)
