...
actual object PlatformStorage : StoragePaths, FileSystem {
    ...
    private fun requireContext(): Context = appContext ?: throw IllegalStateException(
        "PlatformStorage not initialized. Call PlatformStorage.initialize(context) first.",
    )

    actual override fun getModelsDir(): String = File(requireContext().filesDir, "speechkmp_models").absolutePath

    actual override fun getTempDir(): String = File(requireContext().cacheDir, "speechkmp_tmp").absolutePath
