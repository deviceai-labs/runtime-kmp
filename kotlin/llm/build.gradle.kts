...
kotlin {
    ...
    if (isMacOs) {
        ...
        listOf(
            ...
            val compileTask =
                tasks.register(
                    "compileLlmCMake${arch.name.replaceFirstChar { it.uppercase() }}",
                    Exec::class,
                ) {
                    dependsOn(buildTaskName)
                    environment("PATH", "/opt/homebrew/bin:" + System.getenv("PATH"))
                    commandLine =
                        listOf(cmakePath, "--build", cmakeBuildDir.absolutePath, "--target", "llm_static", "--verbose")
                }

            val libPath = cmakeBuildDir.absolutePath
            val mergeTask =
                tasks.register(
                    "mergeLlmStatic${arch.name.replaceFirstChar { it.uppercase() }}",
                    Exec::class,
                ) {
